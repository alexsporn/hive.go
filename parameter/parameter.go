...
// and returns a new viper instance.
//
// It automatically reads in a single config with name defined in "configName"
// and ending with: .json, .toml, .yaml or .yml (in this sequence).
func LoadConfigFile(configDir string, configName string) (*viper.Viper, error) {
	...
	config.SetConfigName(configDir)
	config.AddConfigPath(configName)
